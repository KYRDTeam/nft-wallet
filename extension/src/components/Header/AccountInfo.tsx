...
const AccountInfo = () => {
  ...
  return (
    <Flex alignItems="center" mb={4}>
      <AccountDetailModal
        render={(onOpen) => (
          <Tag px="3" cursor="pointer" onClick={onOpen} fontSize="sm" display="flex" mr={4}>
            {!pageInfo?.domain ? (
              <></>
            ) : trustedApps[account || ""]?.find((item: any) => item.domain === pageInfo?.domain) ? (
              <Text backgroundColor="#1DE9B6" p="1" borderRadius="100%" mr={2}></Text>
            ) : (
              <Text backgroundColor="red.500" p="1" borderRadius="100%" mr={2}></Text>
            )}
      ...
      <Flex>
        ...
        >
          <Image src={Copy} alt="copy icon" w="4" h="4" cursor="pointer" onClick={onCopy} />
        </Tooltip>
      </Flex>
    </Flex>
  );
