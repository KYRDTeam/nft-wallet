...
import { keysSelector, setAccounts, setLoading, setVault, setSelectedAccount, setAccountsName } from "src/store/keys";
...
export const usePersistAccount = () => {
  ...
  const { keyringController, accounts, accountsName, selectedAccount } = useAppSelector(keysSelector);

  useEffect(() => {
    localStore.get().then((data) => {
      if (data.vault) dispatch(setVault(data));
    });
    if (selectedAccount && selectedAccount in accounts) {
  ...
  useEffect(() => {
    accounts.forEach((account) => {
      if (accountsName[account]) {
        dispatch(setAccountsName({ address: account, name: accountsName[account] }));
      } else {
        dispatch(setAccountsName({ address: account, name: "Account " + accounts.indexOf(account) }));
      }
    });
